...
private fun deleteMutedTests(deleteMap: Map<String, MuteTestJson>) {
    for ((_, muteTestJson) in deleteMap) {
        val response =
            khttp.delete("https://buildserver.labs.intellij.net/app/rest/mutes/id:${muteTestJson.id}", headers = headers, auth = authUser)
        checkResponseAndLog(response)
    }
}
...
private data class MuteTestJson(
    val id: Int,
...
private enum class Scope(val id: String, val localDBPath: File, val isBuildType: Boolean) {
    COMMON("Kotlin_BuildPlayground_Jupiter_Tests", File("tests/mute-common.csv"), false),
    IJ193("Kotlin_BuildPlayground_Jupiter_IdePluginTests_IdeaUltimate_193", File("tests/mute-platform.csv"), true),
    IJ192("Kotlin_BuildPlayground_Jupiter_IdePluginTests_IdeaUltimate_192", File("tests/mute-platform.csv.192"), true),
    IJ201("Kotlin_BuildPlayground_Jupiter_IdePluginTests_IdeaUltimate_201", File("tests/mute-platform.csv.201"), true),
    AS36("Kotlin_BuildPlayground_Jupiter_IdePluginTests_IdeaCommunity_as36", File("tests/mute-platform.csv.as36"), true),
    AS40("Kotlin_BuildPlayground_Jupiter_IdePluginTests_StudioMPP_as40", File("tests/mute-platform.csv.as40"), true);
}